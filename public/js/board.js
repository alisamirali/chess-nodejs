let initBoard = (username) => {
  ...
  let illegalWhiteMove = (piece) =>
    gameData.color === "white" && piece.search(/^b/) !== -1;
  let illegalBlackMove = (piece) =>
    gameData.color === "black" && piece.search(/^w/) !== -1;

  function onDragStart(_source, piece, _position, _orientation) {
    if (
      gameOver() ||
      turn === -1 ||
      illegalWhiteMove(piece) ||
      illegalBlackMove(piece)
    ) {
      return false;
    }
  }
